...
import { hoyISO } from '@/lib/app-utils'
...
import { X, GripVertical, CalendarOff } from 'lucide-react'
...
export default function AgendaView() {
  const { pendientes, proyectos, actualizarPendiente } = useApp()
    ...
    <div className="flex h-full gap-3">
      ...
      <div className="flex-1 overflow-y-auto rounded-xl border bg-card p-2 scroll-thin">
        {horas.map(h => {
          ...
          const eventosEnEstaHora = eventosGoogle.filter(e => e.inicio && !e.todoElDia && new Date(e.inicio).getHours() === h)
          return (
            <div key={h} onDragOver={e => e.preventDefault()} onDrop={e => onDrop(h, e)} className="flex min-h-[52px] gap-2 border-b py-1">
              <div className="w-12 shrink-0 pt-1 text-[11px] text-muted-foreground">{String(h).padStart(2, '0')}:00</div>
